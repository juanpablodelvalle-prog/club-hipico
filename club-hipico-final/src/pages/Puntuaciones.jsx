...
export default function Puntuaciones() {
  ...
  // Barrel/Pole
  const [punts, setPunts]       = useState({}) // { iid: { tiempo, elem, saved } }
  ...
  const onComp = async (id) => {
    ...
    setCompSel(c)
    setPunts({}); setObsVals({}); setTReal(''); setTrailIdx(0)
    if (!id) { setRondas([]); setIns([]); return }
    const [{ data: r }, { data: i }] = await Promise.all([
      getRondas(id), getInscripciones(id),
    ])
  ...
  const cargarPunts = async (rid, ins) => {
    ...
    ins.forEach(i => {
      ...
      map[i.id] = p ? { tiempo: p.tiempo_seg, elem: p.elementos_derribados, saved: true } : { tiempo: '', elem: 0, saved: false }
    })
    setPunts(map)
  }
  ...
  const updatePunt = (iid, field, val) => {
    ...
    if (prev?.saved) {
      setConfirm({
        msg: `Ya hay una puntuación guardada para este competidor. ¿Deseas modificarla?`,
        cb: () => {
  ...
  const puntuacionExisteTrail = () => {
    return trailPunts.find(p => p.inscripcion_id === inscActual?.id)
  }

  const guardarTrail = async () => {
    if (!obsOk || !tReal || !inscActual) return
    const prev = puntuacionExisteTrail()
    const doSave = async () => {
      setGuardando(true)
    ...
    if (prev) {
      setConfirm({
        msg: `Ya existe una puntuación para ${inscActual.usuarios?.nombre} — ${inscActual.caballos?.nombre}. ¿Modificar?`,
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:16 }}>
        ...
        <Field label="Ronda" style={{ margin:0 }}>
          <select style={S.fi} value={rondaId} onChange={e => onRonda(e.target.value)}>
      ...
      {compSel && !isTrail && (
        <Card>
          <CardTitle>{compSel.modalidad === 'barrel' ? 'Barrel Racing · +5 s por barril derribado' : 'Pole Bending · +5 s por palo derribado'}</CardTitle>
          {inscripciones.map(ins => {
            const v = punts[ins.id] || { tiempo: '', elem: 0, saved: false }
            const total = v.tiempo ? (parseFloat(v.tiempo) + (parseInt(v.elem)||0) * 5).toFixed(3) : '—'
            return (
              <div key={ins.id} style={{ padding:'12px 0', borderBottom:'1px solid #1e2130' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                  <div>
                    <div style={{ fontSize:13, fontWeight:500, color:'#f0ead6' }}>
                      {ins.usuarios?.nombre} {ins.usuarios?.apellidos}
                    </div>
                    <div style={{ fontSize:11, color:'#6b7280' }}>🐴 {ins.caballos?.nombre}{ins.dorsal ? ` · #${ins.dorsal}` : ''}</div>
                  </div>
                  {v.saved && <span style={{ fontSize:10, padding:'3px 8px', borderRadius:20, background:'#2a1e0a', color:'#fbbf24', fontWeight:500 }}>✓ Guardado</span>}
                </div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
                  <div>
                    <div style={{ fontSize:10, color:'#6b7280', textTransform:'uppercase', marginBottom:3 }}>Tiempo (s)</div>
                    <input
                      style={{ ...S.fi, padding:'10px', fontSize:15, opacity: v.saved ? 0.6 : 1 }}
                  ...
                  <div>
                    <div style={{ fontSize:10, color:'#6b7280', textTransform:'uppercase', marginBottom:3 }}>
                  ...
                  <div>
                    <div style={{ fontSize:10, color:'#6b7280', textTransform:'uppercase', marginBottom:3 }}>Total</div>
                    <div style={{ fontSize:16, fontWeight:600, color:'#c9a96e', padding:'10px 0' }}>{total}</div>
                  </div>
                </div>
                {v.saved && (
                  <button
                    style={{ ...S.bs, fontSize:12, padding:'7px 12px', marginTop:8 }}
                    onClick={() => setConfirm({
                      msg: `¿Deseas modificar la puntuación de ${ins.usuarios?.nombre} — ${ins.caballos?.nombre}?`,
                    ...
                    {SCORES.map(s => (
                      <button key={s} onClick={() => setObsVals(p => ({ ...p, [n]: s }))}
                        style={{
                          ...
                          background: obsVals[n] === s ? (s < 0 ? '#7f1d1d' : s === 0 ? '#374151' : s === 1 ? '#14532d' : s === 2 ? '#1e3a5f' : '#3b1d6e') : '#1c1f2a',
                          color: obsVals[n] === s ? (s < 0 ? '#fca5a5' : s === 0 ? '#d1d5db' : s === 1 ? '#86efac' : s === 2 ? '#93c5fd' : '#c4b5fd') : '#6b7280',
                        }}>
                        {s < 0 ? '−1' : s}
                      </button>
                    ))}
